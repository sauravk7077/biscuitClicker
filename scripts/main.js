...
let props; // All required DOM elements

const biscuitBtn = document.getElementById('biscuit');
const biscuitCountDiv = document.getElementById('biscuitCount'); //Event Listeners

biscuitBtn === null || biscuitBtn === void 0 ? void 0 : biscuitBtn.addEventListener('click', biscuitClick); //Functions
...
function display() {
  ...
  if (shopContainer) {
    ...
    for (let i = 0; i < props.biscuitValues.length; i++) {
      ...
      if (props.biscuitValues[i].cost > props.biscuitCount && !shopBtn.children[0].className.endsWith('disabled')) shopBtn.children[0].className += " disabled";else if (props.biscuitValues[i].cost <= props.biscuitCount) shopBtn.children[0].className = shopBtn.children[0].className.replace(' disabled', '');
...
`;
  ...
  for (let i = 0; i < props.biscuitValues.length; i++) {
    ...
    shopBtn.children[0].addEventListener('click', e => {
      handleShopClick(e, shopBtn.id);
    });
